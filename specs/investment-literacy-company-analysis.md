...
- **Everyday Economics** owns consumer arithmetic, exact compound-interest
  balances, inflation, and simple expected value. This topic owns only the
  Rule-of-72 approximation as a bounded way to interpret a fictional company's
  constant compound growth rate; it does not duplicate exact balance drills.
...
ruleOf72EstimatedDoublingPeriods = 72 / positiveRateInPercent
...
- Rule-of-72 doubling-time estimates for positive constant compound company
  growth rates, paired with explicit approximation limits;
...
- extrapolating a historical growth rate into an unstated forecast or treating
  an estimated doubling time as a prediction, valuation conclusion, or
...
points. For a positive constant compound rate stated as a whole percent per
period, the Rule of 72 estimates doubling periods as `72 ÷ rate`. It is a mental
estimate, not the exact compound-growth solution and not a forecast that the
rate will persist.
...
- Treating `72/r` as an exact doubling time or applying it to a negative,
  changing, non-compound, or period-mismatched rate.
- Treating a historical or supplied scenario rate as a promise of future
  doubling.
...
### Family `rule_of_72_doubling_estimate`

#### Learner task

Estimate the number of periods required for a fictional positive company metric
to double at a supplied constant compound rate, or determine whether the Rule
of 72 is applicable under the stated assumptions.

#### Relationship to the skill

The family builds quick scale intuition around compound growth while repeatedly
requiring the learner to keep an estimate separate from an exact result and
from a forecast. Applicability variations train recognition of the assumptions
that make the shortcut meaningful.

#### Response mode

Decimal-number input labeled in the same period unit as the rate, normally
rounded to one decimal period. Friendly cases may require an exact integer
estimate. Applicability variations use single-choice with stable answers
`applicable_as_estimate` and `not_applicable`, plus one selected reason.

#### Question templates

- `Fictional company {company} has {metric} growing at a supplied constant compound rate of {rate}% per {period}. Using the Rule of 72, estimate the doubling time in {periodPlural}.`
- `{scenario}. Is the Rule of 72 applicable as a rough doubling-time estimate?`
- `The Rule of 72 estimate for {metric} is {estimate} {periodPlural}. What constant compound rate per {period} does that imply?`

Every numeric prompt ends with `This is an arithmetic estimate under the stated
constant-rate scenario, not a forecast or investment conclusion.`

#### Placeholder definitions

- `{company}` is a fictional company identifier generated by the controlled
  company grammar.
- `{metric}` is a positive scale metric whose repeated multiplication is
  meaningful in the scenario, chosen from `revenue`, `active subscriptions`,
  `units sold`, or another locally defined positive driver. It is never share
  price, investor return, profit crossing zero, or a ratio whose doubling lacks
  a stable interpretation.
- `{rate}` is the numeric percent rate, not a decimal fraction. Numeric-estimate
  cases normally use exact decimal rates in `3%..12%`; controlled limitation
  cases may use `1%..2.9%` or `12.1%..25%` only to compare approximation
  quality, never to imply equal accuracy.
- `{period}` is `year`, `quarter`, or `month`; `{periodPlural}` is its matching
  plural. The rate and requested doubling time always use the same period.
  Annualized conversion is never implicit.
- `{scenario}` states the metric, rate behavior, compounding model, and period.
  Applicable scenarios specify one positive constant compound rate. Inapplicable
  scenarios contain exactly one root violation: simple/additive growth,
  changing rates, a zero/negative rate, mismatched periods, an external
  acquisition or discontinuity that breaks the repeated-rate model, or a claim
  that the result is an exact forecast.
- `{estimate}` is a positive Rule-of-72 doubling-time estimate constructed from
  an allowed rate and displayed to no more than one decimal place.

#### Answer template

Forward numeric answer: `{doublingPeriods}`. Inverse numeric answer:
`{rate}% per {period}`. Applicability answer:
`{applicabilityStatus}: {reasonId}`.

#### Answer derivation

For a supplied constant compound rate `r` expressed as percentage points per
period, calculate `doublingPeriods=72/r`. For an inverse prompt, calculate
`r=72/doublingPeriods`. Preserve the exact rational quotient internally and
round only the requested answer using the displayed precision.

The exact compound-model doubling time, used only for validation and limitation
feedback, is `ln(2)/ln(1+r/100)`. It is not the Rule-of-72 answer. Applicability
is true only when the scenario supplies a positive, constant, compound rate for
a consistently measured positive metric and asks for a rough estimate in the
matching period unit. It is false when the one injected violation breaks any of
those conditions or when the claim treats the shortcut as exact/predictive.

#### Accepted answers

Numeric period answers accept a localized decimal with an optional matching
period suffix; a percent sign is rejected. Rate answers accept the global
percent syntax with an optional matching `per {period}` suffix. Answers are
correct when they round to the canonical requested precision; an exact
logarithmic doubling time is not accepted when the prompt explicitly requests
the Rule-of-72 estimate. Applicability choices compare stable IDs and require
the matching root reason when a reason is requested.

#### Instance constraints

- Numeric cases state `positive`, `constant`, and `compound`, and align the rate
  period with the requested answer unit.
- The metric's starting level is positive; its numerical amount is omitted
  unless a controlled variation asks the learner to recognize that the doubling
  estimate does not depend on starting magnitude.
- Every prompt and feedback labels the result an estimate.
- The generator must display these limitations in Learn and worked feedback:
  the shortcut assumes a constant positive compound rate and consistent periods;
  ignores variability, acquisitions/disposals, dilution, fees, taxes, and
  external cash flows where relevant; becomes less reliable far from ordinary
  mid-single- to low-double-digit rates; and does not show that a historical or
  scenario rate will continue.
- No answer is interpreted as company quality, valuation, safety, expected
  security return, or a buy/sell signal.

#### Rejection rules

Reject numeric cases with zero or negative rates, a metric crossing zero,
simple/additive growth, hidden period conversion, or a displayed quotient
requiring more precision than the prompt permits. Reject applicability cases
with no root violation or more than one root violation. Reject cases in which
rounding makes a named misconception identical to the answer, the story implies
a real company or rate, or the result is phrased as a prediction.

#### Controlled variations

- forward estimate from rate to doubling periods;
- inverse estimate from doubling periods to rate;
- compare two supplied constant rates by which has the shorter estimated
  doubling time;
- identify whether one controlled scenario satisfies the approximation's
  assumptions;
- compare the Rule-of-72 estimate with a supplied exact compound-model result
  and calculate absolute estimation error.

All variations preserve the mental operation of applying or qualifying
`72/r`. Exact CAGR calculation remains in `cagr`; exact future-value arithmetic
remains in Everyday Economics.

#### Difficulty levels

- **Level 1:** Whole rates dividing 72 exactly (`4, 6, 8, 9, 12%`), annual
  periods, direct estimate, all assumptions prominently shown.
- **Level 2:** Other whole or half-point rates in `3%..12%`, one-decimal period
  answer, or compare two direct estimates.
- **Level 3:** Inverse rate estimate, nonannual but period-matched rate, or
  recognize one obvious inapplicable scenario.
- **Level 4:** Distinguish approximation from a supplied exact doubling time and
  compute error, or diagnose a subtler changing-rate/period/model violation.
- **Level 5:** Evaluate a claim that combines correct arithmetic with an
  unsupported forecast or company-quality conclusion; return the arithmetic
  estimate and the controlled limitation separately.

#### Multiple-choice distractors

Applicability distractors use: `always exact`, `divide the rate by 72`,
`applicable despite changing/simple/negative growth`, `convert periods without
conversion data`, or the wrong injected reason. Comparison distractors rank the
larger rate as a longer doubling time or compare rate differences without
division. Remove duplicates and any choice entailed by the scenario under a
different but valid reading.

#### Feedback

**Correct feedback.** `Rule-of-72 estimate: 72 ÷ {rate} = {estimate}
{periodPlural}. This is a rough constant-compound-rate estimate, not a
forecast.`

**Incorrect feedback.** If the response matches `rate/72`, identify the reversed
division. If it matches an exact logarithmic result, explain that the question
asked for the shortcut and show both labeled values. For applicability errors,
name the single violated assumption. For predictive claims, confirm any correct
arithmetic but explicitly reject the extrapolation.

**Worked solution.** State the rate as percentage points per matching period,
substitute it in `72/r`, calculate and round once, then list the active
assumptions and one sentence on what cannot be concluded. In comparison mode,
show both quotients. In error mode, show
`|Rule-of-72 estimate−exact compound time|`.

#### Examples

1. `Fictional company Alder has revenue growing at a supplied constant compound rate of 6% per year. Using the Rule of 72, estimate the doubling time.`
   Answer: `12 years`. Derivation: `72/6=12`. Level 1; targets reversed
   division and keeps the result explicitly approximate.
2. `Fictional company Birch has active subscriptions growing at a supplied constant compound rate of 8% per quarter. Estimate the doubling time in quarters.`
   Answer: `9 quarters`. Derivation: `72/8=9`; no annual conversion is needed
   because units match. Level 3; targets period mismatch.
3. `A fictional company's units sold grew 12% last year. An analyst says, “The Rule of 72 gives 6 years, so units will definitely double in 6 years.” Select the valid assessment.`
   Answer: `not_applicable to the predictive conclusion: persistence is
   unsupported`. The arithmetic `72/12=6` is a rough conditional estimate only.
   Level 5; targets forecast overreach.

#### Implementation notes

Store rate as an exact rational number of percentage points and the Rule-of-72
answer as `72/r`. Use a high-precision logarithm implementation only for
limitation/error variants and validation, never as the shortcut oracle. Encode
scenario assumptions as flags and inject exactly one violation in ordinary
applicability items. Structural signatures include direction, rate band, period,
metric, applicability reason, rounding, and estimate-error band.

#### Automated validation

Recompute `72/r` independently; verify inverse round-trip, period-unit equality,
positive metric/rate constraints, and final rounding. For error variants,
recompute `ln(2)/ln(1+r/100)` at greater precision than displayed and assert the
reported error. For applicability cases, compare the scenario flags with the
single expected reason. Verify every placeholder, disclaimer, choice uniqueness,
rejection rule, and misconception distinction over at least 10,000 seeds per
level.

#### Coverage requirements

Across fixed-seed samples, balance forward, inverse, comparison, applicability,
and error modes; whole and half-point rates; years, quarters, and months with
matched units; the ordinary `3%..12%` band and a minority of explicit
limitation-band cases; and every inapplicability reason. At least one third of
advanced items must assess assumptions or interpretation rather than division
alone. No metric, rate, or “faster doubling” case may dominate.
...
Driver arithmetic and one-period growth precede CAGR. Rule-of-72 direct
estimation follows compound-growth-factor fluency; applicability and limitation
items interleave only after the learner labels the result as approximate. Gross
margin precedes operating and net margin. Scenario operating leverage follows
historical reconstruction; audits interleave only after the bases are stable.
...
all components supplied. Friendly direct Rule-of-72 estimates may appear only
after positive compound growth is established.
...
- Rule-of-72 result treated as exact or predictive → paired shortcut/exact
  result plus assumption-classification item;
...
- Rule-of-72 estimates equal exact `72/r` in matching period units; any
  logarithmic exact result is stored separately and never substituted for the
  requested estimate.
...
- Rule-of-72 forward/inverse estimates, matched periods, applicability limits,
  and approximation-versus-forecast distinctions;
...
- Rule-of-72 estimation and limitation checks after compound-growth basics;
...
- [ ] Rule-of-72 answers are labeled estimates, use matched periods, expose
      assumptions and limitations, and never become forecasts or advice.
