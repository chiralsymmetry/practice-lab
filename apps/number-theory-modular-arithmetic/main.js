(function () {
  ...
  function enabledCells(){var cells=[];FAMILIES.forEach(function(family){if(progress.settings.enabled[family.categoryId]===false)return;var levels=PracticeLabUI.unlockedLevels(LEVELS,function(level){return getStat(family.id,level);}),level=levels[levels.length-1];cells.push({family:family,level:level,stat:getStat(family.id,level)});});return cells;}
